class User < ApplicationRecord
  ...
  # module progress
  scope :with_module_progress, -> { joins(:user_module_progresses) }
  ...
  scope :completed_available_modules, -> { with_completed_modules.group('users.id').having('count(user_module_progresses.id) = ?', ModuleRelease.count) }
  ...
  def active_modules
    started_module_names = user_module_progresses.started.pluck(:module_name)
  ...
  # @return [Boolean]
  def course_started?
    user_module_progresses.started.exists?
  end

  # @return [Boolean]
  def module_in_progress?
    user_module_progresses.in_progress.exists?
  end

  # @return [Array<String>]
  def modules_in_progress
    user_module_progresses.in_progress.pluck(:module_name)
  end

  # @param module_name [String]
  # @return [Boolean]
  def module_completed?(module_name)
    user_module_progresses.completed.exists?(module_name: module_name)
  end
