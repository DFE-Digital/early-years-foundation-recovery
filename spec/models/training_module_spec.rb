...
RSpec.describe TrainingModule, type: :model do
  ...
  it 'has fields' do
    expect(training_module.attributes.keys).to eq([:title, :thumbnail, :description, :objective, :criteria, :duration, :summative_assessment_precentage_pass_threshold, :name, :id])
  end
