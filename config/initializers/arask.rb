Arask.setup do |arask|
  ...
  arask.create task: 'db:analytics:users', interval: 10.hours if Rails.env.production?
  arask.create task: 'db:analytics:ahoy_events', interval: 10.hours if Rails.env.production?
  arask.create task: 'db:analytics:user_assessments', interval: 10.hours if Rails.env.production?
  arask.create task: 'db:analytics:user_answers', interval: 10.hours if Rails.env.production?
  arask.create task: 'db:analytics:ahoy_visits', interval: 10.hours if Rails.env.production?
end
