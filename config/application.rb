...
module EarlyYearsFoundationRecovery
  class Application < Rails::Application
    ...
    if Rails.env.test?
      config.service_url = 'http://localhost'
      config.google_cloud_bucket = 'bucket'
      config.dashboard_update_interval = '0 0 */2 * *'
      config.notify_token = 'dummy'
      config.mail_job_interval = '0 12 * * *'
      config.user_password = 'password'
      config.bot_token = 'dummy'
      config.google_analytics_tracking_id = 'dummy'
      config.clarity_tracking_id = 'dummy'
      config.unlock_in_minutes = 120
      config.timeout_in_minutes = 1440
      config.contentful_space = 'dummy'
      config.contentful_delivery_access_token = 'dummy'
      config.contentful_preview_access_token = 'dummy'
      config.contentful_management_access_token = 'dummy'
      config.contentful_environment = 'dummy'
      config.gov_one_base_uri = 'dummy'
      config.gov_one_private_key = 'dummy'
      config.gov_one_client_id = 'dummy'
      config.sentry_dsn = 'dummy'
      config.application_insights_connection_string = 'dummy'
    ...
    config.active_storage.queues.purge            = :default
      ...
      config.contentful_space                   = ENV.fetch('CONTENTFUL_SPACE', 'dummy')
      config.contentful_delivery_access_token   = ENV.fetch('CONTENTFUL_DELIVERY_TOKEN', 'dummy')
      config.contentful_preview_access_token    = ENV.fetch('CONTENTFUL_PREVIEW_TOKEN', 'dummy')
      config.contentful_management_access_token = ENV.fetch('CONTENTFUL_MANAGEMENT_TOKEN', 'dummy') # TODO: use service account management token
      config.contentful_environment             = ENV.fetch('CONTENTFUL_ENVIRONMENT', 'dummy')
